...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
     ...
     *
     * <p>For most applications {@link DiskCacheStrategy#RESOURCE} is ideal. Applications that use
     * the same resource multiple times in multiple sizes and are willing to trade off some speed and
     * disk space in return for lower bandwidth usage may want to consider using {@link
    ...
    /**
     * Sets a resource to display if the model provided to {@link com.bumptech.glide.RequestBuilder#load(Object)}
     * is {@code null}.
     *
     * <p>If a fallback is not set, null models will cause the error drawable to be displayed. If the
     * error drawable is not set, the placeholder will be displayed.
     ...
     *
     * <p>This is a component option specific to {@link VideoDecoder}. If the default video decoder
     ...
     * decoded using a decoder that cannot control the format ({@link android.media.MediaMetadataRetriever}
     * for example), or that the decoder may choose to ignore the requested format if it can't display
     * the image (i.e. RGB_565 is requested, but the image has alpha).
     *
     ...
     * be ignored.
     *
     * <p>To set only the format used when decoding {@link Bitmap}s, use {@link #set(Option,
     * Object)}} and {@link Downsampler#DECODE_FORMAT}. To set only the format used when decoding GIF
     * frames, use {@link #set(Option, Object)} and {@link GifOptions#DECODE_FORMAT}.
     *
     ...
     *
     * <p>{@code #apply} only replaces those values that are explicitly set in the given {@code T}.
     * If you need to completely reset all previously set options, create a new {@code T} object
