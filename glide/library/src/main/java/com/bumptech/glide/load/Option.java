 ...
 *
 * @param <T> The type of the option ({@link Integer}, {@link android.graphics.Bitmap.CompressFormat}
 *            etc.), must implement {@link #equals(Object)} and {@link #hashCode()}.
 */
public final class Option<T> {
    ...
    /**
     * Returns a new {@link Option} that uses the given {@link com.bumptech.glide.load.Option.CacheKeyUpdater}
     * to update disk cache keys.
     *
     * @param key A unique package prefixed {@link String} that identifies this option (must be stable
     *            across builds, so {@link Class#getName()} should <em>not</em> be used).
     */
    ...
    /**
     * Returns a new {@link Option} that uses the given {@link com.bumptech.glide.load.Option.CacheKeyUpdater}
     * to update disk cache keys and provides the given value as the default value.
     *
     * @param key A unique package prefixed {@link String} that identifies this option (must be stable
     *            across builds, so {@link Class#getName()} should <em>not</em> be used).
     */
    ...
    public interface CacheKeyUpdater<T> {
         ...
         * class and use {@link Option#memory(String)} or {@link Option#memory(String, Object)}
         * instead.
         *
         ...
         *                 MessageDigest#update(byte[])} by all implementations if the digest is updated
         *                 with the given {@code value} parameter.
         * @param value    The value of of this particular option. Typically you should convert the
         *                 value to a byte array using some stable mechanism and then call {@link
