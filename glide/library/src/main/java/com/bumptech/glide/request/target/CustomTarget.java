 ...
 *
 * <p>You <em>MUST</em> implement {@link #onLoadCleared(Drawable)} and ensure that all references
 * to any resource passed into the target in {@link #onResourceReady(Object, Transition)} are
 * removed before {@link #onLoadCleared(Drawable)} completes. Failing to do so can result in
 * graphical corruption, crashes caused by recycled {@link Bitmap}s, and other undefined behavior.
 * It is never safe to leave {@link #onLoadCleared(Drawable)} unimplemented or empty. Even if you do
 * not manually clear this {@link Target}, Glide may do so automatically after certain lifecycle
 * events in {@link androidx.fragment.app.Fragment}s and {@link android.app.Activity}s.
 *
...
public abstract class CustomTarget<T> implements Target<T> {
    ...
    /**
     * Creates a new {@link CustomTarget} that will attempt to load the resource in its original
     * size.
     *
     ...
     * as the requested size (unless overridden by {@link com.bumptech.glide.request.RequestOptions#override(int)}
     * in the request).
     *
